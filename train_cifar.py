...
# Model detail
parser.add_argument('--model', default='ms_resnet18_qs', type=str, metavar='MODEL',
                    help='Name of model to train (default: "countception"')
parser.add_argument('--T', type=int, default=4, metavar='time',
...
def main():
    ...
    if args.distributed:
        ...
        # pynvml.nvmlInit()
        # global gpu_info_handle
        # gpu_info_handle = pynvml.nvmlDeviceGetHandleByIndex(args.device) 
    assert args.rank >= 0
    ...
    if 'ms_resnet' in args.model:
        from model import ms_resnet_cifar
        model = ms_resnet_cifar.__dict__[args.model](T=args.T,num_classes=args.num_classes,T_c_config=args.T_c_config)      
    elif 'Spikingformer' in args.model:
        from model import spikingformer_cifar
        model = create_model(
            args.model,
            pretrained=False,
            drop_rate=0.,
            ...
            depths=args.depths, sr_ratios=1,T=args.T,T_c_config=args.T_c_config
    ) 
    else:
    ...
    print(f"Creating model, the Time Step is {args.T}!")
    ...
    if args.rank == 0:
        ...
        with open(os.path.join(output_dir, 'args.yaml'), 'w') as f:
            f.write(args_text)
        FileHandler = logging.FileHandler(os.path.join(output_dir, 'logger.log'))
    ...
    formatted_time = str(timedelta(seconds=total_time_seconds))
    formatted_time = formatted_time.split('.')[0]
    if best_metric is not None:
        _logger.info(' | Best metric: {0} (epoch {1}) | Total Training Time Cost: {2} |'.format(best_metric, best_epoch,formatted_time))
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                # global gpu_info_handle
                # gpu_info = pynvml.nvmlDeviceGetMemoryInfo(gpu_info_handle)
                _logger.info(
                    'Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                    'Loss: {loss.val:>9.6f} ({loss.avg:>6.4f})  '
                    'Time: {batch_time.val:.3f}s, {rate:>7.2f}/s  '
                    '({batch_time.avg:.3f}s, {rate_avg:>7.2f}/s)  '
                    'Peak Memory: {memory:.0f}  '
                    # 'Used Memory: {used_memory:.0f}  '
                    'LR: {lr:.3e}  '
                    'Data: {data_time.val:.3f} ({data_time.avg:.3f})'.format(
                        ...
                        memory = torch.cuda.max_memory_allocated(device=args.device) / MB,
                        # used_memory = gpu_info.used / MB,
