...
from model import ms_resnet_dvs
import  utils
...
import autoaugment
from model import spikingformer_dvs
_seed_ = 2021
import random
random.seed(2021)
...
def parse_args():
    ...
    parser.add_argument('--model', default='Spikingformer', help='model')
    parser.add_argument('--T_c_config', type=int, nargs='+', default=[None,None,None,None,None],)
    parser.add_argument('--dataset', default='cifar10dvs', help='dataset')
    parser.add_argument('--num-classes', type=int, default=10, metavar='N',
                        help='number of label classes (default: 1000)')
    parser.add_argument('--data-path', default='/home/ys/data/cifar10_dvs/', help='dataset')
    ...
    parser.add_argument('--resume_no_opt', action='store_true')
    parser.add_argument(
        "--sync-bn",
        dest="sync_bn",
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None, T_train=None, aug=None, trival_aug=None, mixup_fn=None):
    ...
    metric_logger.add_meter('imgps', utils.SmoothedValue(window_size=10, fmt='{value}'))
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # image torch.Size([16, 16, 2, 128, 128])
        ...
        metric_logger.meters['imgps'].update(batch_size / (time.time() - start_time))

    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print(f'training speed: img/s = {metric_logger.imgps.global_avg}')
...
def main(args):
    ...
    output_dir = os.path.join(args.output_dir, f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}_{args.model}_b{args.batch_size}_T{args.T}')
    ...
    from model import resnet_dvs
    if args.model in ms_resnet_dvs.__dict__:
        model = ms_resnet_dvs.__dict__[args.model](T=args.T, num_classes=args.num_classes,T_c_config=args.T_c_config)
    
    elif args.model in resnet_dvs.__dict__:
        model = resnet_dvs.__dict__[args.model](num_classes=args.num_classes)
    elif args.model in spikingformer_dvs.__dict__:
        model = create_model(
            args.model,
            pretrained=False,
            T=args.T,
            ...
            num_classes=args.num_classes,T_c_config=args.T_c_config
        )
    else:
    ...
    train_snn_aug = transforms.Compose([
                    # transforms.Resize((52,52),interpolation=transforms.InterpolationMode.NEAREST),
